...
- Features seguem o Spec Kit: `/speckit-specify` → `/speckit-plan` → `/speckit-tasks` →
  `/speckit-implement`, com um diretório de spec por feature em numeração sequencial.
- O "Constitution Check" do plano MUST avaliar os Princípios I–VII; qualquer violação MUST ser
  registrada na tabela de complexidade com justificativa e alternativa mais simples rejeitada.
...
- Emendas são feitas via `/speckit-constitution`, com Sync Impact Report no topo do arquivo,
...
- Versionamento semântico: MAJOR para remoção ou redefinição incompatível de princípio; MINOR
  ...
  e redação.
- Conformidade é verificada no Constitution Check de cada plano, em `/speckit-analyze` e em
  revisão de código; complexidade adicional MUST ser justificada por escrito.
