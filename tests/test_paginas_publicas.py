...
def test_matriculas_hierarquica_participa_da_ordenacao_compartilhada(matriculas_com_dados):
    _, matriz = matriculas_com_dados.atualizar(
    ...
    assert getattr(tabelas[0], "data-sortable", None) == "true"
    cabecalhos = [c for c in componentes(tabelas[0]) if type(c).__name__ == "Th"]
    assert getattr(cabecalhos[1], "data-no-sort", None) == "true"
...
def test_evasao_mostra_percentual_e_texto_da_faixa_com_a_classe_de_cor(evasao_com_dados):
    _, tabela = evasao_com_dados.atualizar("com_fic", ["campus"], "__todos__", "__todos__")
    assert _celulas_de_evasao(tabela) == {
        ...
        "Total": ("20,0% (Média)", "evasao-media num"),
    }


def test_evasao_mostra_taxa_agregada_destacada_antes_da_tabela_publica(evasao_com_dados):
    kpi, tabela = evasao_com_dados.atualizar("com_fic", ["campus"], "__todos__", "__todos__")
...
def test_evasao_recorte_vazio_nao_exibe_cartao_zero_e_limpar_preserva_sem_fic(evasao_com_dados):
    kpi, aviso = evasao_com_dados.atualizar("sem_fic", ["campus"], "Inexistente", "__todos__")
    assert textos(kpi) == ""
    assert "Sem dados para os filtros selecionados." in textos(aviso)
    valores_limpos = evasao_com_dados.limpar_filtros(1)
    assert valores_limpos == ("__todos__", "__todos__", "sem_fic", ["campus"])
