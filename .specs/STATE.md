...
- **Feature**: `.specs/features/cards-atualizar-dados`
- **Phase / Task**: concluída e validada (escopo Medium, sem `design.md`/`tasks.md` formais — "Tasks implícito no Execute").
- **Completed**: a tela "Atualizar dados" troca o `<fieldset>` de rádio "De onde vêm os dados" por dois cards sempre visíveis lado a lado (`flex-lg-row`, empilhados <992px): "Atualizar do Sistec" e "Enviar pastas", cada um com sua explicação, ação e áreas de resultado/preservação próprias (`app/templates/atualizar.html`). Classe própria `.card-atualizar` (`app/assets/style.css`), só com tokens de superfície do DS (`--card-padding`, `--surface-shadow-sm`, `--surface-rounder-md`) — nunca `.br-card` (`position:absolute` no pacote vendorizado, quebraria o layout). `atualizar.js` perdeu só a lógica de alternância de origem (`escolherOrigem` e os listeners de `change`). Commit `cc777b7` (implementação) + `4eb8243` (spec/validation).
- **Verificação final**: `.specs/features/cards-atualizar-dados/validation.md` — PASS, na 2ª rodada. Verifier (Codex via `codex:codex-rescue`) achou CAD-03 não atendida na 1ª implementação: áreas de resultado/preservação do envio tinham saído do card (para uma `<section id="atualizar-resultados">` externa que também reparentava Progresso/Prévia, fora de escopo); corrigido no mesmo diff antes de commitar. `pytest tests/ -q`: 680 passed.
- **Execução**: implementada e corrigida diretamente por mim (Claude, Sonnet 5) nesta sessão — sem `handoff-ds`/DeepSeek desta vez.
- **Não fazer sem OK explícito**: `git push`, merge da branch e deploy. Nada foi enviado ao remoto.
- **Pendência fora do escopo**: as páginas do Dash não renderizam com query string (`/matriculas?x=1`); falta token CSRF nos POST administrativos (registrado em `CUTOVER.md`, pré-existente). `scripts/verificar_prontidao_cutover.py` segue em NO-GO nesta máquina só por `ADMIN_EMAIL`/`ADMIN_PASSWORD_HASH`/`CALCSISTEC_HTTPS` não configurados (ambiente local, não código). A tela reorganizada em cards ainda não foi conferida visualmente pela usuária no navegador real — vale confirmar antes de considerar encerrado.
- **Working tree**: diretórios locais não rastreados `.agents/`, `.verifier-scratch-t9/` e o arquivo solto `nonascii.txt`; não os incluir sem conferir a origem.
