...
2. WHILE the viewport is at least 992px wide (the two cards side by side) THE result block SHALL still span the full width below both cards, not just the width of one card.
   > **Como esta AC é garantida — e como ela é verificada.** A garantia é **estrutural**: nenhuma regra de `app/assets/style.css` limita a largura desses blocos (as únicas com `max-width` no arquivo são `.container-fluid` a partir de 1600px — que vale para a área de conteúdo inteira, do mesmo jeito para Progresso e Prévia —, `pre`, `.br-button` e `.br-message .content`); o que dá largura inteira ao bloco é ele não estar dentro de nenhum container com largura de coluna, e o container `d-flex flex-column flex-lg-row` dos dois cards é o único do trecho. Por isso o teste é o recorte estrutural de `tests/test_tela_atualizar_envio.py` (bloco fora do container flex dos cards, na mesma faixa de `#atualizar-progresso`/`#atualizar-previa`), não uma medição de pixels: largura renderizada em ≥992px é checagem de layout do navegador, fora do alcance do `test_client` do Flask — confirmar a largura real na tela é da conferência visual, não de um teste `pytest`.
